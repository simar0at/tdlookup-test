...
describe("TDView", function(){
	...
	describe("Translation of type strings to some better readable english", function(){
		it("should translate the 5 entity types annotated to some better readable english", function() {
			_.forEach(['plant', 'illness', 'astrEnt', 'auxSubst', 'textGenre'], function(type){
				get_user_readable_text_for_type(type).should.not.equal(type);
			});
		});
		it("should translate a string like land__name_of_people to 'A land or name of people'", function(){
			get_user_readable_text_for_type('land__name_of_people').should.equal('A land or name of people');
		});
	});
		...
		it("should attach an item annotation", function(done){
			loadFixture('zamad.html').done(function(){
				...
				TDView.attachTagData(function(){
					...
					texts[1].textContent.should.not.equal('auxSubst');
					texts[2].textContent.should.equal('bandage; medical application; ointment', 'Item english translation is wrong!');
					//texts[3].textContent.should.equal('fascia', 'Item latin translation is wrong!');
			    	done();					
				});
				requests[0].respond(200, {
					"Content-Type": "application/json" 
				...
				requests[1].respond(200, {
                    ...
                    '{ "item" : { "xml:id" : "uni_darling_d27e198398", "type": "auxSubst", "name" : { "xml:lang" : "ota-Latn-t", "type" : "preferred", "#text" : "żamad" }, "cit" : { "type" : "translation", "sense" : { "xml:lang" : "en-UK", "#text" : "bandage; medical application; ointment" }}, "note" : "correct version: żımād" }, "count" : "1" }');
			});								
		})
	});
});
